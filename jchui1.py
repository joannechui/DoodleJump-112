...
def generatePlatforms(app):
    app.limit = int(app.base + abs(app.scrollY))
    newPlatforms = random.randint(1, 4)
    bounds = int(app.scrollMargin)
...
def selectMovingPlatforms(app):
    #choose random amount of platforms
    numPlats = random.randint(0, 3)
    movingPlat = []
    while numPlats > 0:
        platIndex = random.randint(0, len(app.platforms)-1)
        movingPlat.append(app.platforms.pop(platIndex))
        numPlats -= 1
    movePlatform(app, movingPlat)
1
def movePlatform(app, movingPlat):
    for platX, platY in movingPlat:
        platX += app.xv*app.direction
        if (platX + app.platWidth >= app.width or 
            platX <= 1):
            app.direction = -app.direction
        print(platX, platY, 'asd')
        app.platforms.append((platX, platY))
...
def getPlatform(app):
    ...
    for (platX, platY) in app.platforms:
...
def getRandomPlat(app):
    ...
    startX, startY = app.platforms[index]
...
def monsterJump(app):
    for platX, platY in app.platforms:
        ...
        # print('dist=', dist)
        # print('jumpheight=', app.jumpHeight)
        # print(app.platforms)
